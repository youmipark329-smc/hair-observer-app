...
> 배포 전·수집 데이터 없음을 확인하고 수행했으며, 앱 내부 식별자도 함께 변경(IndexedDB `hair_observer`, SW 캐시 `hair-observer-v21` — 현재값(앱 v1.18). 버전을 올릴 때는 `app.js`(APP_VERSION)·`index.html`(#verApp 폴백)·`sw.js`(CACHE) 세 곳을 함께 바꾸고 `node verify_version.js` 로 확인한다).
...
- **(v1.17) 환자 익명 ID 는 목록에서 고릅니다:** 시작화면의 **[— 사전배정 목록에서 선택 —]**
  ...
  목록에 없는 환자면 맨 아래 **[✎ 목록에 없음 — 직접 입력]** 을 골라 **숫자 3자리만** 칩니다
  (접두사 `P-16E-` 는 병동 드롭다운에서 자동으로 만들어집니다. `7` 만 쳐도 `007` 로 채워집니다).
